    ...
    for image_idx in range(start_image_idx, train_dataset.num_images):
        ...
        for name, param in pipeline.named_parameters():
            if param.shape == torch.Size([47737, 1]):
                # read the csv file and assign it to a torch vector and then move that torch vector to gpu device and then assign to param.data
                param.data = torch.from_numpy(np.loadtxt('/home/swarm_researchers/Research/Darshit/personal/Trans_INR/trans-inr/codebk.csv')).to(device)
